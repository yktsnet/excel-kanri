...
07 は PR #27 が「Issue ファイルの open コミットのみ」の状態でマージ・close されており、**実装は行われていない**（tape/GIF の最終変更は #7 のまま）。本 Issue は 07 と同一要件の再実行。**PR 作成前に `git diff --name-only main` に上記『対象』のファイルが含まれることを必ず確認すること。**
